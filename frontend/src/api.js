import axios from 'axios';

const API_BASE = process.env.REACT_APP_API_URL || 'http://localhost:8000';

const api = axios.create({ baseURL: API_BASE });

export const getStats = () => api.get('/api/stats').then(r => r.data);
export const getMatches = (params) => api.get('/api/matches/', { params }).then(r => r.data);
export const getMatchDetail = (matchId) => api.get(`/api/matches/${matchId}/detail`).then(r => r.data);
export const getLeaderboard = (limit = 20) => api.get('/api/teams/leaderboard', { params: { limit } }).then(r => r.data);
export const getTeams = () => api.get('/api/teams/').then(r => r.data);
export const getTeamProfile = (name) => api.get(`/api/teams/${name}`).then(r => r.data);
export const getTeamLineup = (name) => api.get(`/api/teams/${name}/lineup`).then(r => r.data);
export const getTeamHeroes = (name) => api.get(`/api/teams/${name}/heroes`).then(r => r.data);
export const searchPlayers = (search, limit = 20) => api.get('/api/players/', { params: { search, limit } }).then(r => r.data);
export const searchPlayersByTeam = (team, search, limit = 50) => api.get('/api/players/', { params: { team, search: search || undefined, limit } }).then(r => r.data);
export const getPlayerProfile = (id) => api.get(`/api/players/${id}`).then(r => r.data);
export const predict = (team1, team2, matchId) => api.post('/api/predictions/predict', { team1, team2, match_id: matchId }).then(r => r.data);
export const backtest = (team1, team2, limit) => api.post('/api/predictions/backtest', { team1, team2, limit }).then(r => r.data);
export const getH2H = (team1, team2) => api.post('/api/h2h/', { team1, team2 }).then(r => r.data);

export const getTournaments = () => api.get('/api/tournaments/').then(r => r.data);
export const getTournamentDetail = (id) => api.get(`/api/tournaments/${id}`).then(r => r.data);
export const getTournamentStandings = (id) => api.get(`/api/tournaments/${id}/standings`).then(r => r.data);
export const analyzeLineup = (player_ids) => api.post('/api/lineup/analyze', { player_ids }).then(r => r.data);

export const getHeroes = (params) => api.get('/api/heroes/', { params }).then(r => r.data);
export const getHeroDetail = (id) => api.get(`/api/heroes/${id}`).then(r => r.data);
export const getHeroMatches = (id, params) => api.get(`/api/heroes/${id}/matches`, { params }).then(r => r.data);
export const getFetchStatus = () => api.get('/api/fetch/status').then(r => r.data);
export const getVerificationStatus = () => api.get('/api/verification/status').then(r => r.data);
export const runVerification = () => api.post('/api/verification/run').then(r => r.data);
export const globalSearch = (q, limit = 5) => api.get('/api/search/', { params: { q, limit } }).then(r => r.data);

export default api;
